...
def remover_agua(source_distance):
	if (get_gromacs_version() == "5.0.2"):
		...
		process = Popen([program, 'select', '-f', 'water.gro', '-s', 'water.tpr', '-on', 'remover.ndx', '-select', 'not same residue as resname SOL and within '+source_distance+' of group Protein'], stdout=PIPE, stderr=PIPE)
		stdout, stderr = process.communicate()
		process2 = Popen([program, 'trjconv', '-f', 'water.gro', '-s', 'water.tpr', '-n', 'remover.ndx', '-o', 'water_ok.gro'], stdout=PIPE, stderr=PIPE) #, '-ignh'
		stdout2, stderr2 = process2.communicate()
	else:
		gmx_path = "/home/"+user+"/programs/gmx-4.6.5/no_mpi/bin/"
		program = os.path.join(gmx_path, "g_select")
		process = Popen([program, '-f', 'water.gro', '-s', 'water.tpr', '-on', 'remover.ndx', '-select', 'not same residue as resname SOL and within '+source_distance+' of group Protein'], stdout=PIPE, stderr=PIPE)
		stdout, stderr = process.communicate()
		program2 = os.path.join(gmx_path, "trjconv")
		process2 = Popen([program2, '-f', 'water.gro', '-s', 'water.tpr', '-n', 'remover.ndx', '-o', 'water_ok.gro'], stdout=PIPE, stderr=PIPE) #, '-ignh'
		stdout2, stderr2 = process2.communicate()
	...
	#os.system('head -n-1 top.top > top_temporary') # top_temporary é top.top sem a última linha, que tem a quantidade de águas
	...
	#os.system('total_aguas=$(grep SOL water_ok.gro | grep OW | wc -l | awk "{print $1}")') # conta o número de águas em water_ok.gro
	...
	while True:
		...
		if not line:
			...
			if line.find("SOL") != -1 and line.find("OW") != -1:
				...
				continue
	arq.close()
	#os.system('echo "SOL              ""$total_aguas" >> top_temporary')		# Adiciona a última linha corrigida ao top_temporary
	arq_top_temp.write("SOL              "+str(cont))
	arq_top_temp.close()
	#os.system('mv top_temporary top.top')		# Troca top_temporary por top.top, que agora tem o número de águas atualizado
