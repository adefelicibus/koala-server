...
def min_none(source_name):
	if (get_gromacs_version() == "5.0.2"):
		...
		process = Popen([program, 'grompp', '-f', 'mdp.mdp', '-c', 'gro.gro', '-p', 'top.top', '-o', 'min_none.tpr'], stdout=PIPE, stderr=PIPE)
		stdout, stderr = process.communicate()
		process2 = Popen([program, 'mdrun', '-s', 'min_none.tpr', '-v', '-deffnm', source_name], stdout=PIPE, stderr=PIPE)
		stdout2, stderr2 = process2.communicate()
	else:
		gmx_path = "/home/"+user+"/programs/gmx-4.6.5/no_mpi/bin/"
		program = os.path.join(gmx_path, "grompp")
		process = Popen([program, '-f', 'mdp.mdp', '-c', 'gro.gro', '-p', 'top.top', '-o', 'min_none.tpr'], stdout=PIPE, stderr=PIPE)
		stdout, stderr = process.communicate()
		program2 = os.path.join(gmx_path, "mdrun")
		process2 = Popen([program2, '-s', 'min_none.tpr', '-v', '-deffnm', source_name], stdout=PIPE, stderr=PIPE)	
		stdout2, stderr2 = process2.communicate()
