...
def structure_not_accepted_by_min_all(grofile, stderr):
	...
	arq = open(log_file_structures_not_accepted_by_min_all)
	while True:
		line = arq.readline()
		if not line:
			break  # EOF
...
def min_all(source_name):
	if (get_gromacs_version() == "5.0.2"):
		...
		process = Popen([program, 'grompp', '-f', 'mdp.mdp', '-c', 'gro.gro', '-p', 'top.top', '-o', 'min_all.tpr'], stdout=PIPE, stderr=PIPE)
		stdout, stderr = process.communicate()
		process2 = Popen([program, 'mdrun', '-s', 'min_all.tpr', '-v', '-deffnm', source_name], stdout=PIPE, stderr=PIPE)
		stdout2, stderr2 = process2.communicate()
	else:
		gmx_path = "/home/"+user+"/programs/gmx-4.6.5/no_mpi/bin/"
		program = os.path.join(gmx_path, "grompp")
		process = Popen([program, '-f', 'mdp.mdp', '-c', 'gro.gro', '-p', 'top.top', '-o', 'min_all.tpr'], stdout=PIPE, stderr=PIPE)
		stdout, stderr = process.communicate()
		program2 = os.path.join(gmx_path, "mdrun")
		process2 = Popen([program2, '-s', 'min_all.tpr', '-v', '-deffnm', source_name], stdout=PIPE, stderr=PIPE)	
		stdout2, stderr2 = process2.communicate()
	
	#Checking output of min_all of grofile
	if stderr.find('Fatal error') >= 0:	
		structure_not_accepted_by_min_all('gro.gro', stderr)
	...
	if stderr2.find('Fatal error') >= 0:
		structure_not_accepted_by_min_all('gro.gro', stderr2)
		for raiz, diretorios, arquivos in os.walk(os.getcwd()):
			for arquivo in arquivos:
				if not arquivo.endswith(".log") and not arquivo.endswith(".gro"):
					os.remove(os.path.join(raiz,arquivo))
