...
def pdb2gmx(pdbfile, forcefield, water, ignh):
	if (get_gromacs_version() == "5.0.2"):
		...
		process = Popen([program, 'pdb2gmx', '-f', pdbfile, '-o', 'prot', '-p', 'top', ignh, '-ff', forcefield, '-water', water], stdout=PIPE, stderr=PIPE) #, '-ignh'
	else:
		gmx_path = "/home/"+user+"/programs/gmx-4.6.5/no_mpi/bin/"
		program = os.path.join(gmx_path, "pdb2gmx")
		process = Popen([program, '-f', pdbfile, '-o', 'prot', '-p', 'top', ignh, '-ff', forcefield, '-water', water], stdout=PIPE, stderr=PIPE) #, '-ignh'
