...
def genion(concentration):
	if (get_gromacs_version() == "5.0.2"):
		...
		process = Popen([program, 'genion', '-s', 'tpr.tpr', '-o', 'gro.gro', '-p', 'top.top', '-pname', 'NA', '-nname', 'CL', '-pq', '1', '-nq', '-1', '-neutral', '-conc', concentration], stdin=PIPE, stdout=PIPE, stderr=PIPE)
	else:
		gmx_path = "/home/"+user+"/programs/gmx-4.6.5/no_mpi/bin/"
		program = os.path.join(gmx_path, "genion")
		process = Popen([program, '-s', 'tpr.tpr', '-o', 'gro.gro', '-p', 'top.top', '-pname', 'NA', '-nname', 'CL', '-pq', '1', '-nq', '-1', '-neutral', '-conc', concentration], stdin=PIPE, stdout=PIPE, stderr=PIPE)
