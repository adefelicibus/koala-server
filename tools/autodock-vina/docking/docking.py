...
def load_parameters():
    ...
    parser.add_argument("-f", action="store", dest="file_output1", required=True)
    # Parameter for log file
    parser.add_argument("-g", action="store", dest="file_output2", required=True)
...
def run_docking(autodock_vina, config_box, ligand, receptor, structure_file, log_file):
    process = Popen([
        ...
        structure_file,
        '--log',
        log_file], stdout=PIPE, stderr=PIPE)
